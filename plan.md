...
1. Criar projeto de testes xUnit em `tests/Freight.Tests` e adicionar referência para os projetos alvo (principalmente `Freight.Application`).
...
- `tests/Freight.Tests/Freight.Tests.csproj` — novo; adicionar referências e pacotes.
- `src/Freight.Application/Freight.Application.csproj` — referenciar no projeto de testes.
- Workspace tasks — opcionalmente adicionar `dotnet test` task no VS Code tasks.json.

**Verification**

1. Executar `dotnet test tests/Freight.Tests/Freight.Tests.csproj --collect:"XPlat Code Coverage"` e confirmar saída com `coverage.cobertura.xml` ou `coverage.info`.
